...
import { type Athlete, type Coach, type Organization, rosterApi } from '@/api'
import { useAppDispatch, useAppSelector } from '@/core/hooks'
import { useDefaultOrganizationId } from '@/modules/settings'
import { type Loadable } from '@/shared/types'
import { getLoadableStatusFromQuery } from '@/shared/utils'

import { activateAthlete, selectActivatedAthlete } from '../store/roster'
import { getAthleteSlug } from '../utils/getAthleteSlug'

interface UseRosterFeatureResult extends Loadable {
  activatedAthlete: Athlete | null
  activatedAthleteSlug: string | null
  athletes: Athlete[]
  coach: Coach | null
...
export function useRosterFeature(): UseRosterFeatureResult {
  ...
  const activatedAthleteSlug = activatedAthlete ? getAthleteSlug(activatedAthlete) : null
