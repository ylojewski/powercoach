...
import { type GetCurrentSettingsApiResponse } from '@/api'
...
describe('useSettingsFeature', () => {
  ...
  it('exposes the default organization id through the public read hook', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(createSettingsResponse()))

    const { result } = renderHook(
      () => ({
        defaultOrganizationId: useDefaultOrganizationId(),
        settings: useSettingsFeature()
      }),
      { wrapper: createWrapper() }
    )

    act(() => {
      result.current.settings.load()
    })

    await waitFor(() => {
      expect(result.current.defaultOrganizationId).toBe(settingsResponse.defaultOrganizationId)
    })
  })
